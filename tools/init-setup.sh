#!/usr/bin/env bash
#
# tools/init-setup.sh - One-Step Setup for VaultWarden-OCI-Slim
#
...
# Robustly determine the script's and repository root's location.
...
setup_project() {
    ...
    if [[ ! -f "$REPO_ROOT/settings.env" && -f "$REPO_ROOT/settings.env.example" ]]; then
        cp "$REPO_ROOT/settings.env.example" "$REPO_ROOT/settings.env"; chmod 600 "$REPO_ROOT/settings.env"
...
setup_sqlite_maintenance() {
    ...
    if [[ ! -f "$REPO_ROOT/sqlite-maintenance.sh" ]]; then
    ...
    if [[ "$setup_maint" =~ ^[Yy]$ ]]; then
        chmod +x "$REPO_ROOT/sqlite-maintenance.sh"
        if "$REPO_ROOT/sqlite-maintenance.sh" --schedule "0 3 * * 0"; then
...
EOF
    ...
    if [[ -f "$TOOLS_DIR/settings.env.wizard.sh" ]]; then
        chmod +x "$TOOLS_DIR/settings.env.wizard.sh"
        "$TOOLS_DIR/settings.env.wizard.sh"
    else
        log_warning "Configuration wizard not found. Please configure settings.env manually."
    fi
    
    log_step "Setup Complete!"
    log_success "Your system is now ready. Review settings.env and run ./startup.sh from the project root."
}
